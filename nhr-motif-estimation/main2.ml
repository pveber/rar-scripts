...
let nhre_matrix h1 h2 (orientation, spacer) = 
  let bg = Pwm.flat_background () in
  Pwm.tandem ~orientation ~spacer h1 h2 bg
...
let matches_of_sequence h1 h2 theta s = 
  let annot motif sense (i,s) = (motif, sense, i, s) in
  selected_motifs
  |> List.map 
      List.(fun motif ->
	let mat = nhre_matrix h1 h2 motif in
...
let compile_fragments fragments = 
  let r1 = Array.(init 6 (fun _ -> make 4 0)) 
  and r2 = Array.(init 6 (fun _ -> make 4 0)) in
  let foreach (u,v) =
    let f r s = 
      String.iteri 
	(fun i c -> 
...
let theta_estimation_step tol h1 h2 = 
  let matches = 
    List.map (matches_of_sequence h1 h2 5.) sequences
  and control_matches = 
    List.map (matches_of_sequence h1 h2 5.) control_sequences
  in
...
let estimation_step tol h1 h2 =
  let theta, matches, control_matches = theta_estimation_step tol h1 h2 in
  ...
  let fragments = 
    List.map2
      (fun seq matches ->
	List.map 
	  (fun (motif,sense,i,_) -> extract_hexamers motif sense seq i) 
	  matches)
...
let rec estimation n tol h1 h2 = 
  if n <= 0 then Tuple3.first (theta_estimation_step tol h1 h2), h1, h2
  else 
    let h1, h2 = estimation_step tol h1 h2 in
    estimation (n - 1) tol h1 h2
...
let theta_hat, h1_hat, h2_hat = estimation 1 0.1 balmer_counts balmer_counts
let _ = 
  print_matrix (freq_matrix h1_hat) ;
  print_matrix (freq_matrix h2_hat)
...
let () =
  ...
  let mat = nhre_matrix h1_hat h2_hat (`direct, 0) in
  let l =
    List.filter
      (fun s -> Pwm.fast_scan mat s theta_hat <> [])
      seqs in
  printf "nb dodecamers compatible with theta = %f: %d\n" theta_hat (List.length l)

let () = 
  let mat = nhre_matrix h1_hat h2_hat (`direct, 1) in
  List.iter
    (fun seq -> 
